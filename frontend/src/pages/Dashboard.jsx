      ...
      <header className="bg-[#FFE500] h-[52px] flex items-center justify-between px-4 md:px-12">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Link to="/profile" className="hidden md:flex items-center gap-2 hover:opacity-80 transition-opacity">
            {currentUser?.picture ? (
              <img src={currentUser.picture.startsWith('/api') ? `${BACKEND_URL}${currentUser.picture}` : currentUser.picture} alt={currentUser.name} className="w-8 h-8 rounded-full object-cover" />
            ) : (
